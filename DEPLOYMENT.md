...
1. Open `https://<your-domain>.vercel.app/api/ask` in your browser
2. **Expected result**: You should see JSON like `{ "ok": true, "route": "/api/ask", "message": "GET ping works", ... }`
3. **If you see 404**: The route doesn't exist, which means Root Directory is wrong

**Fix steps:**
1. Go to Vercel Dashboard → Your Project → Settings → General
2. Scroll to **"Root Directory"**
3. **Set Root Directory to**: The folder containing `package.json` (usually `./` for root, or leave blank)
4. **Important**: After changing Root Directory, you **must redeploy**
   - Go to Deployments tab
   - Click "..." on the latest deployment → "Redeploy"

**Production vs Preview:**
- Environment variables must be set for **both Production AND Preview** if you're using preview deployments
- Go to Settings → Environment Variables
- Ensure variables are checked for both environments
